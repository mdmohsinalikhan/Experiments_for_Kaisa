...
with open(filename) as f:
    capacity = [(2**(-1*float(x)))*2**16 for x in f.read().splitlines()]
...
df = 225
support = np.array(range(150,350))
y = [1 / (2*math.gamma(df/2)) * (z/2)**(df/2-1) * math.exp(-z/2) for z in support]
plt.plot(support, y, label='$\chi^2(225)$', linewidth=1.0, linestyle="-", c = 'black', alpha = 1.0)
