...
unsigned long long int ***countKeyVsOutput(unsigned char **plaintext_states, unsigned char **keys,unsigned int **trails)
{
	...
	int total_outputs = pow(2,NOFOB); /* this is the number of total possible output values for which we would like to compute the capacity. 
	...
	for(i = 0; i < NOOFKEYS; i++)
	{
		...
		chooseAndAssignithKey(keys,i); /*a masterkey will be generated with replacement i.e, it may generate a key which has been used before. 
...
void postRoundSSABookKeping(int round,unsigned char **plaintext_states,int key_round_zero, unsigned long long int ***key_vs_output_count,unsigned int **trails)
{
	...
	value_at_input_output_of_trail = value_at_input_of_trail ^ value_at_output_of_trail; // xoring input and output bits of the trail
...
long double capacity_20th_round = 0.0;
	...
	for(i = 0; i <= ROUND;i++)
	{
		...
		for(j = 0; j < total_keys; j++)
		{
			...
			if(i == 20)
			{
				capacity_20th_round = ssa_capacities[i][j][0]*pow(2,16);
				printf("%.12Lf \n", capacity_20th_round);
			}
		}
	}
...
long double mean = 0.0, variance = 0.0;
	...
	for(i = 0;i <= ROUND;i++)
	{
		mean = -1*log(ssa_capacities_mean_variance_table[i][0])*LOG2E;
		variance = -1*log(ssa_capacities_mean_variance_table[i][1])*LOG2E;
		printf("%d	%.8Lf	%.12Lf \n",i,mean,variance);
	}
